...
export const publicAssets=['golf-cart.jpg','avirob-isaac-sim.mp4','avirob-isaac-sim-poster.jpg','racing-loop.mp4','racing-poster.jpg','icra-qualification.mp4','early-robot.jpeg','early-pid.mp4','wro-demo-silent-125.mp4','wro-demo-poster.jpg','wro-robot.jpeg','roscon-stage.jpg','control-response.svg','control-cascade.svg','control-latency.svg'];
const evidenceFiles=['pid-benchmark.csv','pid-position.csv','pid-cascade.csv'];
export function build(){
  const root=resolve(import.meta.dirname,'..');
  const pages=renderSite(root);
  const context=contextDocuments(pages);
  ...
  for(const destination of [root,root+'/dist']){
    writeFileSync(destination+'/llms.txt',context.index);
    writeFileSync(destination+'/llms-full.txt',context.full);
  ...
  const allowed=new Set([...pages.keys(),'styles.css','app.js','favicon.svg','sitemap.xml','robots.txt','.nojekyll','llms.txt','llms-full.txt',...publicAssets.map(f=>'assets/'+f),...evidenceFiles.map(f=>'evidence/'+f)]);
